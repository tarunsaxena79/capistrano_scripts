...
old_commit_id=`cat /home/<user>/CURRENT_REVISION`
	...
	#Mail Acknowledgement of the deployment status
		...
		then 
        	( echo "Subject: Deployemnt Successful on the All the Servers" ; echo "The new Git commit ID $commit_id has been successfully deployed  on all the servers under ELB.The IP of the servers are $mail_IP_format" ) | /usr/sbin/sendmail -F Deployment-Alert <emailID>
        	else
        	( echo "Subject: Deployemnt failed on the servers" ; echo "Deployment failed on the servers--Rollbacking to the previous commit ID - $old_commit_id" ) | /usr/sbin/sendmail -F Deployment-Alert <emailID>
